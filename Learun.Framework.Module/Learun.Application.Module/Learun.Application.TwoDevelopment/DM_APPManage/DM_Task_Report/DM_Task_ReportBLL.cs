...
namespace Learun.Application.TwoDevelopment.DM_APPManage
{
    ...
    /// 描 述：任务举报记录
    /// </summary>
    public class DM_Task_ReportBLL : DM_Task_ReportIBLL
    {
        private DM_Task_ReportService dM_Task_ReportService = new DM_Task_ReportService();
        ...
        public IEnumerable<dm_task_reportEntity> GetList( string queryJson )
        {
            try
            {
                return dM_Task_ReportService.GetList(queryJson);
            }
            catch (Exception ex)
            {
        ...
        public dm_task_reportEntity GetEntity(int keyValue)
        {
            try
            {
                return dM_Task_ReportService.GetEntity(keyValue);
            }
            catch (Exception ex)
            {
                if (ex is ExceptionEx)
        ...
        public void DeleteEntity(int keyValue)
        {
            try
            {
                dM_Task_ReportService.DeleteEntity(keyValue);
            }
            catch (Exception ex)
            {
                if (ex is ExceptionEx)
        ...
        public void SaveEntity(int keyValue, dm_task_reportEntity entity)
        {
            try
            {
                dM_Task_ReportService.SaveEntity(keyValue, entity);
            }
            catch (Exception ex)
            {
                if (ex is ExceptionEx)
