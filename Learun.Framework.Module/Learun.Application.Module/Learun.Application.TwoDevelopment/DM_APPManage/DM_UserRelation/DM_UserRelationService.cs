...
namespace Learun.Application.TwoDevelopment.DM_APPManage
{
    public class DM_UserRelationService : RepositoryFactory
    {
        ...
        public DataTable GetMySonChildDetail(int user_id, int PageNo, int PageSize)
        {
            try
            {
                string querySonChildSql = "select u.nickname,u.headpic,insert(u.phone, 4, 4, '****') as phone,u.userlevel,u.createtime,ur.user_id,ur.ordercount,ur.ReviceTaskCount taskcount,ur.partners_id from dm_user_relation ur left join dm_user u on ur.user_id=u.id where ur.parent_id in(select user_id from dm_user_relation where parent_id=" + user_id + ")";
        ...
        public DataTable GetPartnersChildDetail(int? partners_id, int PageNo, int PageSize)
        {
            try
            {
                string queryPartnersChildSql = "select u.nickname,u.headpic,insert(u.phone, 4, 4, '****') as phone,u.userlevel,u.createtime,ur.user_id,ur.ordercount,ur.ReviceTaskCount taskcount,ur.partners_id from dm_user_relation ur left join dm_user u on ur.user_id=u.id where ur.partners_id=" + partners_id;
